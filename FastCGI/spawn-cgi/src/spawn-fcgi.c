...
static int fcgi_spawn_connection(char *appPath, char **appArgv, int fcgi_fd, int fork_count, int child_count, int pid_fd, int nofork) {
	int status, rc = 0;
	struct timeval tv = { 0, 100 * 1000 }; //默认是100ms
	...
	while (fork_count-- > 0) {
		...
		case 0: {
			...
			printf("FCGI_LISTENSOCK_FILENO is:%d\n", FCGI_LISTENSOCK_FILENO);

			if(fcgi_fd != FCGI_LISTENSOCK_FILENO) {
				close(FCGI_LISTENSOCK_FILENO);
				dup2(fcgi_fd, FCGI_LISTENSOCK_FILENO); // 将FCGI_LISTENSOCK_FILENO这个文件描述符重定向到了fcgi_fd
				close(fcgi_fd);
			}
		...
		default:
			...
			/* wait */
			select(0, NULL, NULL, NULL, &tv); //阻塞tv=100ms
...
int main(int argc, char **argv) {
	...
	// "c:d:f:g:?hna:p:u:vC:F:s:P:U:G:M:S" 选项字符串
	// 冒号表示参数，一个冒号就表示这个选项后面必须带有参数（没有带参数会报错哦），但是这个参数可以和选项连在一起写，也可以用空格隔开
	// 两个冒号的就表示这个选项的参数是可选的，即可以有参数，也可以没有参数，但要注意有参数时，参数与选项之间不能有空格（有空格会报错的哦），这一点和一个冒号时是有区别的。
	// cgi用法：./spawn-fcgi -f /usr/local/services/file_cloud_cgi_cos40-1.0/cgibin/cgi_file_cloud_handler_coss3 -F 50 -a 127.0.0.1 -p 8004
