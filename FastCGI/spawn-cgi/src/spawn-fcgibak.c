...
static int fcgi_spawn_connection(char *appPath, int fcgi_fd, int fork_count) {
    int status, rc = 0;
    struct timeval tv = { 0, 100 * 1000 }; //默认100ms
    pid_t child;
...
int main(int argc, char **argv) {
    ...
    while (-1 != (o = getopt(argc, argv, "f:p:F:"))) {
        ...
        case 'p': port = strtol(optarg, &endptr, 10);/* port */ // &endptr不为NULL，则会将遇到的不符合条件而终止的字符指针由 endptr 传回；
            if (*endptr) {
    ...
    fcgi_spawn_connection(fcgi_app, fcgi_fd, fork_count);
