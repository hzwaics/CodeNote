...
int FCGX_IsCGI(void)
{
    if (isFastCGI != -1) {  // isFastCGI初始默认就是-1
        return !isFastCGI;
    }

    if (!libInitialized) { // libInitialized初始默认是0，第一次需要进入初始化
        int rc = FCGX_Init();
        if (rc) {
...
int FCGX_Init(void)
{
    ...
    // 初始化全局变量 the_request；指定listenfd=FCGI_LISTENSOCK_FILENO，flags=0，ipcFd=-1
    FCGX_InitRequest(&the_request, FCGI_LISTENSOCK_FILENO, 0); 

    if (OS_LibInit(NULL) == -1) {   // 参考os_unix.c中的详细实现
